...
class FakePiForward extends EventEmitter {
  ...
  private readonly waiters: Array<
    (sent: { toPc: string; env: Envelope }) => void
  > = [];
...
describe("agent-network e2e", () => {
  ...
  test("2) two agents request/reply — orq.request(backend) → pong", async () => {
    ...
    backend.onMessage((env: Envelope) => {
      void backend
        .send(env.from, { reply_to: env.id, status: "ok", text: "pong" })
        .then(() => undefined)
        .catch(() => undefined);
    ...
    backend2.onMessage(async (env) => {
      ...
      const sockets = (
        backend2 as unknown as { socket: import("node:net").Socket | null }
      ).socket;
  ...
  test("3) parallel wave — Promise.all([req(be), req(fe)]) — both respond", async () => {
    ...
    async function autoReply(p: SessionPeer, replyText: string) {
      p.onMessage(async (env) => {
        ...
        const s = (p as unknown as { socket: import("node:net").Socket | null })
          .socket;
  ...
  test("broadcast: msg pra todos exceto sender", async () => {
    ...
    a.onMessage((e) => {
      if (
        ...
        e.body &&
        (e.body as { type?: string }).type !== "peer_joined" &&
        (e.body as { type?: string }).type !== "peer_left"
      )
        inboxA.push(e);
    });
    b.onMessage((e) => {
...
describe("ACK protocol (plan/25 Wave 0)", () => {
  ...
  test("sendWithAck resolves on cross-PC ACK (from=<pc>:broker)", async () => {
    ...
    try {
      const pendingAck = orq.sendWithAck(
        "trab:agent-1",
        { task: "ping" },
        null,
        1500,
    ...
    test.each(invalidExactAckBodies)(
      ...
      async ({ body }) => {
        ...
        try {
          ...
          expect(
            broker.injectFromRemote(
              envelope("casa:broker", peer.address(), body, outbound.id),
            ),
          ).toBe("received");
          await flushUds();
  ...
  test("trusted offline transport error immediately resolves sendWithAck with reason", async () => {
    ...
    try {
      ...
      const pending = orq
        .sendWithAck("trab:agent-1", { task: "ping" }, null, 1000)
        .then((value) => {
          result = value;
          return value;
        });
  ...
  test("trusted broker transport error immediately rejects request once with MeshTransportError", async () => {
    ...
    try {
      ...
      const pending = observe(
        orq.request("trab:agent-1", { task: "ping" }, 1000),
      ).then((outcome) => {
        settlements += 1;
        return outcome;
      });
  ...
  test("local and remote forged transport error messages settle neither pending operation", async () => {
    ...
    try {
      ...
      const ackOutcome = observe(
        orq.sendWithAck("trab:agent-1", { task: "ack" }, null, 1000),
      ).then((outcome) => {
        ackSettled = true;
        return outcome;
      });
      const ackOutbound = await capture.nextOutbound();
      const requestOutcome = observe(
        orq.request("trab:agent-2", { task: "request" }, 1000),
      ).then((outcome) => {
        requestSettled = true;
        return outcome;
      });
  ...
  test.each([
    ...
    {
      compatibility: "malformed 32-hex correlation id",
      malformed: "re" as const,
    },
    { compatibility: "genuine silence", malformed: null },
  ])(
    "$compatibility transport error cannot bypass the strict Broker boundary",
    async ({ malformed }) => {
      const sock = tmpSock();
      const orq = await makePeer(sock, "orq");
      const broker = orq.localBroker()!;
  ...
  test("Relay offline errors cross BrokerRemote and real UDS with trusted legacy id normalization", async () => {
    ...
    try {
      ...
      fakePi.emit(
        "envelope",
        envelope("old-rtx:_broker_remote", "old-mac:_broker_remote", {
          type: "peers_update",
          peers: ["remote-agent"],
          peers_detailed: [
      ...
      async function startSend(caseName: string): Promise<{
        ...
        const pending = peer
          .sendWithAck("RTX4090:remote-agent", { case: caseName }, null, 1_000)
          .then((value) => {
            result = value;
            return value;
          });
  ...
  test("plan/34: injectFromRemote delivers new work and replies alike (no busy)", async () => {
    ...
    const newWork = {
      from: "casa:sess-3",
      to: "backend",
      id: "01976000-0000-7000-8000-000000000001",
      re: null,
      body: { task: "do thing" },
    };
    ...
    const reply = {
      from: "casa:sess-3",
      to: "backend",
      id: "01976000-0000-7000-8000-000000000002",
      re: "01976000-0000-7000-8000-000000000003",
      body: { answer: 42 },
    };
  ...
  test("injectFromRemote: unknown local peer → denied", async () => {
    ...
    const env = {
      from: "casa:sess-3",
      to: "no-such-peer",
      id: "01976000-0000-7000-8000-000000000004",
      re: null,
      body: { x: 1 },
    };
  ...
  test("Broker.list_peers surfaces remote peers_detailed with pc (plan/38 Fase 2)", async () => {
    ...
    const localEntry = body.peers_detailed.find(
      (p) => p.address === "/w/orq@orq",
    );
  ...
  test("audit.jsonl tags injectFromRemote envelopes with via=relay", async () => {
    ...
    const env = {
      from: "casa:sess-3",
      to: "orq",
      id: "01976000-0000-7000-8000-aaaaaaaaaaac",
      re: null,
      body: { task: "remote ping" },
    };
  ...
  test("unregistered list_peers probe returns the roster without joining", async () => {
    ...
    orq.onMessage((env) => {
      if (env.from === "broker") orqSystem.push(env);
    });
    await wait(50);
    orqSystem.length = 0;
    ...
    const reply = await new Promise<Envelope>((resolve, reject) => {
      ...
      probe.on("connect", () =>
        probe.write(JSON.stringify({ type: "list_peers" }) + "\n"),
      );
    ...
    expect(peers.peers).toEqual(
      expect.arrayContaining(["orq", "backend", "casa:sess-9"]),
    );
...
describe("plan/38 — address encoder + name migration (pure)", () => {
  test("composeAddress renders [<pc>:]<cwd>@<nome>; cwd-less → name", () => {
    ...
    expect(
      composeAddress({ cwd: "/Users/jacob/acme/backend", name: "backend" }),
    ).toBe("/Users/jacob/acme/backend@backend");
    expect(
      composeAddress({ cwd: "/Users/jacob/acme/backend", name: "reviewer" }),
    ).toBe("/Users/jacob/acme/backend@reviewer");
    expect(
      composeAddress({ cwd: "/Users/jacob/.wt/feat-login", name: "backend" }),
    ).toBe("/Users/jacob/.wt/feat-login@backend");
    expect(composeAddress({ cwd: "/x/backend", name: "backend#2" })).toBe(
      "/x/backend@backend#2",
    ); // collision suffix is part of the name
    expect(
      composeAddress({
        pc: "MacMini",
        cwd: "/Users/jose/work/acme",
        name: "app",
      }),
...
describe("plan/38 — (cwd, name) mesh addressing (e2e)", () => {
  ...
  test("register accepts absolute POSIX, Windows drive, and UNC cwd values", async () => {
    ...
    const uncPeer = await makePeerCwd(
      sock,
      "unc",
  ...
  test("registration rejects relative, control-character, and oversized cwd atomically", async () => {
    ...
    const rejected = async (cwd: unknown): Promise<void> => {
      await new Promise<void>((resolve, reject) => {
        ...
        client.on("connect", () =>
          client.write(
            JSON.stringify({ type: "register", name: "bad", cwd }) + "\n",
          ),
        );
        client.on("close", () => {
          clearTimeout(timer);
  ...
  test("registration enforces shared code-unit roster limits before mutating peers", async () => {
    ...
    expect((roster.body as { peers?: string[] }).peers).toContain(
      original.address(),
    );
  ...
  test("takeoverExisting replaces the same cwd/name instead of creating #2", async () => {
    ...
    await expect(first.send("broadcast", { stale: true })).rejects.toThrow(
      "session peer not connected",
    );
    await first.leave();
  ...
  test("list_peers_reply carries peers_detailed ({cwd,name,address})", async () => {
    ...
    expect(body.peers).toEqual(
      expect.arrayContaining(["/a/backend@backend", "/a/web@web"]),
    );
    expect(body.peers_detailed).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
  ...
  test("Windows drive local address beats RemoteRouter alias handling", async () => {
    ...
    const tryRouteOutbound = vi.fn(
      (env: Envelope) => typeof env.to === "string" && env.to.startsWith("C:"),
    );
    broker.setRemoteRouter({
      tryRouteOutbound,
      listRemotePeers: () => [],
    ...
    const ack = await sender.sendWithAck(localTarget.address(), {
      local: true,
    });
    await wait(50);
  ...
  test("broadcast is scoped to the sender's cwd (folder colleagues only)", async () => {
    ...
    sameFolder.onMessage((e) => {
      if (e.from !== "broker") gotSame.push(e);
    });
    otherFolder.onMessage((e) => {
      if (e.from !== "broker") gotOther.push(e);
    });
    ...
    await a.leave();
    await sameFolder.leave();
    await otherFolder.leave();
  });
});
...
describe("SessionPeer.rename (live rename)", () => {
  test("renames in place — no `#N` duplicate, no stale old name (single rejoin)", async () => {
    ...
    const names = ((reply.body as { peers?: string[] }).peers ?? []).filter(
      (x) => x === "alice" || x.startsWith("bob"),
    );
