...
describe("unbienStateHome precedence", () => {
  test("defaults to ~/.pi/un-bien when nothing is set", () => {
    delete process.env["UNBIEN_DIR"];
    delete process.env["UNBIEN_HOME"];
    expect(unbienStateHome()).toBe(join(homedir(), ".pi", "un-bien"));
  });
  ...
  test("UNBIEN_DIR is an absolute override with no .pi/un-bien suffix", () => {
  ...
  test("an empty UNBIEN_DIR falls through to UNBIEN_HOME", () => {
  ...
  test("resolved at call time (a later env change is picked up)", () => {
    process.env["UNBIEN_DIR"] = "/first";
    expect(unbienStateHome()).toBe("/first");
    process.env["UNBIEN_DIR"] = "/second";
...
describe("unbienConfigHome precedence", () => {
  ...
  test("PI_CODING_AGENT_DIR wins over the state resolver (UNBIEN_DIR)", () => {
    process.env["PI_CODING_AGENT_DIR"] = "/agent/home";
    process.env["UNBIEN_DIR"] = "/state/root"; // steers state, not config
  ...
  test("an empty PI_CODING_AGENT_DIR → ~/.pi/extensions (no state-dir fallback)", () => {
    ...
    process.env["UNBIEN_DIR"] = "/state/root";
    expect(unbienConfigHome()).toBe(join(homedir(), ".pi", "extensions"));
  });
