...
import { makeTestBusEmitter, runTestScenario } from "./commands/test_scenarios.js"
import { routeRpcCommand, routeUnBienPlane, type PlaneRouterDeps } from "./routing.js"
...
/**
 * Plane routers live in ./routing.ts (carved, slice 4). These thin wrappers
 * preserve the call sites + the CommandDeps surface; the deps accessors
 * keep module state read LIVE at each dispatch.
 */
function _routeRpcCommandFrom(
  sender: PlainPeerChannel,
  env: EnvelopeMessage,
): void {
  routeRpcCommand(planeDeps, sender, env)
}
function _routeUnBienPlaneFrom(
  sender: PlainPeerChannel,
  env: EnvelopeMessage,
): void {
  routeUnBienPlane(planeDeps, sender, env)
}

// ── Plane-router deps (accessors over module state — live at dispatch) ──────
const planeDeps: PlaneRouterDeps = {
  ...
  liveCtx: () => _liveCtx(),
  pi: () => _pi,
  // Getter: rpcDeps is declared later in the file; the dispatch reads it
  // live at call time.
  get rpcDeps() {
    return rpcDeps
  },
