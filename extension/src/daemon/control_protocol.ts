/**
 * CLI ↔ supervisor IPC contract for `~/.pi/un-bien/supervisor.sock`.
 *
...
export interface DaemonInfo {
  ...
  name: string; // from <cwd>/.pi/un-bien/config.json agent_name
  state: DaemonState;
...
export function parseRequest(line: string): ControlRequest {
  ...
  try {
    ...
    throw new Error(`malformed control request: ${(e as Error).message}`);
  }
...
export function parseReply(line: string): ControlReply<unknown> {
  ...
  try {
    ...
    throw new Error(`malformed control reply: ${(e as Error).message}`);
  }
  if (!obj || typeof obj !== "object") {
