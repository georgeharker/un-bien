...
describe("rpcSpawnArgs", () => {
  ...
  test("always passes --approve (pi >=0.79 project trust; RPC is non-interactive)", () => {
    ...
    expect(rpcSpawnArgs("/path/to/dist/index.js", "PC", false)).toContain(
      "--approve",
    );
  });
});
...
describe("RpcChild — deliberate stop is not a crash", () => {
  ...
  test.skipIf(process.platform === "win32")(
    "stop() emits isCrash:false though the child dies by signal",
    async () => {
      ...
      const exited = new Promise<RpcChildExitEvent>((resolve) =>
...
describe("resolvePiBin (plan/40 — Windows pi.cmd)", () => {
  test("POSIX → returns the bin name unchanged", () => {
    ...
    expect(resolvePiBin("/opt/homebrew/bin/pi", "darwin")).toBe(
      "/opt/homebrew/bin/pi",
    );
  });
  test("Windows → an explicit path or suffixed name is used as-is (no lookup)", () => {
    expect(resolvePiBin("C:\\tools\\pi.cmd", "win32")).toBe(
      "C:\\tools\\pi.cmd",
    );
...
describe("resolvePiSpawn (plan/40 — directly-spawnable target)", () => {
  test("POSIX → command is pi, no prefix args", () => {
    expect(resolvePiSpawn("pi", "linux")).toEqual({
      command: "pi",
      prefixArgs: [],
    });
    expect(resolvePiSpawn("/usr/bin/pi", "darwin")).toEqual({
      command: "/usr/bin/pi",
      prefixArgs: [],
    });
  });
...
describe("RpcChild — isBusy", () => {
  ...
  test.skipIf(process.platform === "win32")(
    "refreshBusy syncs from get_state.isStreaming (authoritative)",
    async () => {
